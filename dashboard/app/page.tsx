...
import { ArrowRight, BarChart3, Box, Cloud, Search, Server, ShieldCheck, TrendingDown, Zap } from 'lucide-react'
...
export default function Home() {
  ...
  const [activeScene, setActiveScene] = useState<'overview' | 'inventory' | 'kubernetes'>('overview')
  ...
  useEffect(() => {
    const order: Array<'overview' | 'inventory' | 'kubernetes'> = ['overview', 'inventory', 'kubernetes']
          ...
          <div className="p-6">
            <div className="mb-5 flex flex-wrap gap-2">
              {[
                ...
                { id: 'inventory' as const, label: 'Cloud Resources', icon: Server },
                { id: 'kubernetes' as const, label: 'K8s Namespaces', icon: Box },
              ].map(scene => {
              ...
              <div className="rounded-xl border border-slate-100 dark:border-slate-800 p-4">
                ...
                <div className="space-y-2">
                  {INVENTORY_ROWS.map((row) => (
                    <div key={row.name} className="grid grid-cols-12 items-center rounded-lg border border-slate-200 px-3 py-2 text-xs dark:border-slate-700">
                      <div className="col-span-4 font-medium text-slate-900 dark:text-white">{row.name}</div>
                      <div className="col-span-2 text-slate-500">{row.type}</div>
                      <div className="col-span-2 text-slate-500">{row.provider}</div>
                      <div className="col-span-2 text-slate-500">{row.region}</div>
