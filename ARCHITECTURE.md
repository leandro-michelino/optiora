...
│    Virtual Tags · Resource-Level Rightsizing · Sustainability Metrics        │
│    Tagging Coverage · Cross-Provider Comparison · Chargeback/Showback        │
...
│    optimization-portfolio · tagging-coverage · sustainability                │
│    cross-provider-comparison · anomaly-intelligence · chargeback             │
│  FinOps Intelligence                                                         │
...
```text
  ...
  - stress envelopes                                  build_tagging_coverage_analytics()
  - backtesting (MAPE)      - action plan signals     build_sustainability_metrics()
...
Path B: Backend narratives (server-side, async-capable)
  finops_mcp/tools/genai_advisor.py
      generate_spend_narrative()          ← monthly spend summary
      generate_anomaly_explanation()      ← per-anomaly root-cause narration
      generate_optimization_brief()       ← CFO-level optimization summary
      generate_maturity_narrative()       ← FinOps maturity plain-language
      generate_budget_risk_alert()        ← budget breach risk narration
      generate_waste_insights()           ← waste category prioritization
      generate_optimization_roadmap()     ← 30/60/90-day action plan
      generate_executive_narrative()      ← board-level 4-paragraph summary
      generate_commitment_strategy()      ← commitment purchase memo
      generate_tagging_strategy()         ← tagging enforcement action plan
      generate_sustainability_narrative() ← carbon footprint brief
      generate_chargeback_narrative()     ← chargeback/showback finance report
      generate_cross_provider_comparison_brief()  ← multi-cloud efficiency brief
      generate_alert_triage()             ← batch alert classification & priority
      generate_rightsizing_brief()        ← rightsizing ROI brief
      generate_vendor_negotiation_brief() ← contract negotiation talking points
...
Fallback model: when OCI GenAI is not configured, all generators return
  (None, prompt) — the prompt is surfaced to the frontend Cost Advisor
  so the user can copy-paste into any external LLM. No hard dependency on OCI GenAI.
```
...
Client (Cost Advisor page)
      ...
      └─► GET /api/v1/advisor/hybrid?narrative_type=optimization_roadmap
              ...
              │    ├─ tagging coverage (allocation readiness score)
              │    ├─ cross-provider comparison (health scores + arbitrage)
              ...
              └─ Advisory block  (GenAI overlay — narrative only)
                   ...
                   ├─ executive_narrative
                   ├─ tagging_strategy (if tagging gap detected)
                   └─ sustainability_narrative (if sustainability context present)
...
Transport:
  ...
  Rate limits  — login / password-reset: 8 attempts / 900 s (process-local;
                 stale buckets purged every 60 s to prevent memory growth;
