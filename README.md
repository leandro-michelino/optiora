...
- deterministic forecast sensitivity stress tests (`GET /api/v1/forecast/sensitivity`) with growth/volatility elasticity and risk windows
- optimization portfolio sequencing (`GET /api/v1/analytics/optimization-portfolio`) with risk-adjusted NPV, payback, and 30/60/90-day phases
...
- GenAI copilot packs (`POST /api/v1/genai/copilot-pack`) that produce multi-audience narratives from deterministic analytics context, including forecast sensitivity and optimization portfolio briefs
...
- `GET /api/v1/forecast/sensitivity` (growth/volatility stress testing, elasticity, and downside risk windows)
...
- `GET /api/v1/analytics/optimization-portfolio` (risk-adjusted FinOps initiative portfolio with sequencing and NPV)
...
- `POST /api/v1/genai/analyze` (backend OCI GenAI narration: `spend`, `anomaly`, `optimization`, `maturity`, `budget_risk`, `waste_insights`, `optimization_roadmap`, `executive_narrative`, `commitment_strategy`, `forecast_sensitivity`, `optimization_portfolio`, `stakeholder_brief`)
- `POST /api/v1/genai/copilot-pack` (single-call, multi-narrative package with deterministic context + advisory overlays, including stakeholder finance/engineering/operations briefs)
